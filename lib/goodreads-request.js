...
RequestManager.get = function (req) {
  ...
  return new Promise(function (resolve, reject) {
    request(path, function (error, response, body) {
      if (error) reject(error);else if (!body) reject();else {
        xmlParser.parseString(body, function (err, result) {
          if (err) reject(XMLError(err.error, 'RequestManager.get()'));else if (result.error) reject(APIError(result.error, 'RequestManager.get()'));else resolve(result.GoodreadsResponse);
        });
      }
    });
  });
    ...
    oauth.get(path, access_token, access_token_secret, function (error, response) {
      if (error) reject(error);else {
        xmlParser.parseString(response, function (err, result) {
          if (err) reject(XMLError(err.error, 'RequestManager.oAuthGet()'));else resolve(result.GoodreadsResponse);
        });
      }
    });
  });
...
RequestManager.oAuthPost = function (req) {
  ...
  return new Promise(function (resolve, reject) {
    oauth.post(path, access_token, access_token_secret, null, null, function (error, response) {
      if (error) reject(error);else {
        xmlParser.parseString(response, function (err, result) {
          if (err) reject(XMLError(err.error, 'RequestManager.oAuthPost()'));else resolve(result.GoodreadsResponse);
        });
      }
    });
  });
...
RequestManager.oAuthDelete = function (req) {
  ...
  return new Promise(function (resolve, reject) {
    oauth.delete(path, access_token, access_token_secret, function (error, response) {
      if (error) reject(error);else {
        xmlParser.parseString(response, function (err, result) {
          if (err) reject(XMLError(err.error, 'RequestManager.oAuthDelete()'));else resolve();
        });
      }
    });
  });
